...
namespace DnDGen.DungeonGen.Tests.Unit.Generators.ExitGenerators
{
    [TestFixture]
    public class ChamberExitGeneratorTests
    {
        ...
        {
            var exit = new Area();
            mockHallGenerator.Setup(g => g.Generate(9266, specifications)).Returns(new[] { exit });

            var exits = chamberExitGenerator.Generate(9266, specifications, 42, 600);
        ...
        public void GetMultipleExits()
        {
            ...
            mockHallGenerator.SetupSequence(g => g.Generate(9266, specifications)).Returns(new[] { exit }).Returns(new[] { otherExit });
        ...
        public void IfAreaBiggerThanLimit_AddExtraExit(int originalExits, int totalExits)
        {
            ...
            mockHallGenerator.Setup(g => g.Generate(9266, specifications)).Returns(() => new[] { new Area() });
        ...
        public void GetDoor()
        {
            ...
            mockDoorGenerator.Setup(g => g.Generate(9266, specifications)).Returns(new[] { door });
        ...
        public void GetOnlyExitLocationForDoor()
        {
            ...
            var door = new Area { Type = AreaTypeConstants.Door };
            mockDoorGenerator.Setup(g => g.Generate(9266, specifications)).Returns(new[] { door });
            mockPercentileSelector.Setup(s => s.SelectFrom(Config.Name, TableNameConstants.ExitLocation)).Returns("on the ceiling");
            mockPercentileSelector.Setup(s => s.SelectFrom(Config.Name, TableNameConstants.ExitDirection)).Returns("to the right");
        ...
        public void GetExitLocationsAndDirections()
        {
            ...
            mockPercentileSelector.SetupSequence(s => s.SelectFrom(Config.Name, TableNameConstants.ExitLocation)).Returns("on the ceiling").Returns("behind you");
            mockPercentileSelector.SetupSequence(s => s.SelectFrom(Config.Name, TableNameConstants.ExitDirection)).Returns("to the right").Returns("to the left");
